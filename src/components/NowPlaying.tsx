...
export function NowPlaying() {
  const { currentSong, progress, currentTime, duration, autoplay, volume, hasPrevious, hasNext, toggle, seek, seekToProgress, playPrevious, playNext, setAutoplay, setVolume, isPlaying } = useAudioPlayer()
  ...
  return <section className="sticky bottom-4 z-40 mt-8 overflow-hidden rounded-[24px] border border-aurora-teal/25 bg-[rgba(6,7,22,.93)] shadow-[0_22px_80px_rgba(0,0,0,.60)] backdrop-blur-2xl">
    <div className="relative grid gap-4 p-4 sm:grid-cols-[158px_1fr] sm:p-5">
      ...
      <div className="min-w-0 self-center">
        ...
        <div className="mt-2 flex items-center gap-2.5"><button type="button" onClick={()=>setVolume(volume > 0 ? 0 : 0.7)} aria-label={volume > 0 ? "Mute" : "Unmute"} className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-starlight/10 text-[#A9A3CC] hover:border-nebula-violet/40 hover:text-starlight">{volume > 0 ? <Volume2 size={15}/> : <VolumeX size={15}/>}</button><input type="range" min="0" max="100" step="1" value={Math.round(volume*100)} onChange={(e)=>setVolume(Number(e.target.value)/100)} aria-label="Volume" className="h-2 w-full max-w-[180px] cursor-pointer accent-[#4FD8C4]"/><span className="min-w-[34px] font-mono text-[9.5px] text-[#777294]">{Math.round(volume*100)}%</span></div>
