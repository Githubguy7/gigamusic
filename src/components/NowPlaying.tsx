...
const THEMES: ThemeName[] = ['Grace & Mercy', 'Heavenly Glory', 'Holy Fire', 'Light & Truth', 'Living Water', 'Cross & Creation']
...
function visualTheme(title: string, scripture?: string | null, meaning?: string | null, lyrics?: string | null, id?: string): ThemeName {
  ...
  if (source.includes('glory') || source.includes('heaven') || source.includes('crown') || source.includes('kingdom')) return 'Heavenly Glory'
  if (source.includes('fire') || source.includes('spirit') || source.includes('pentecost') || source.includes('flame')) return 'Holy Fire'
  if (source.includes('light') || source.includes('truth') || source.includes('word') || source.includes('scripture')) return 'Light & Truth'
  if (source.includes('water') || source.includes('river') || source.includes('thirst') || source.includes('fountain')) return 'Living Water'
  if (source.includes('cross') || source.includes('calvary') || source.includes('creation') || source.includes('earth')) return 'Cross & Creation'
...
function ThemeVisual({ theme, playing }: { theme: ThemeName; playing: boolean }) {
  if (theme === 'Holy Fire') {
    return <div className="relative z-10 flex h-[92px] w-[92px] items-end justify-center" aria-hidden="true"><div className={`absolute bottom-2 h-16 w-12 rounded-[50%_50%_46%_46%] bg-[radial-gradient(circle_at_50%_75%,rgba(255,231,160,0.95),rgba(244,136,63,0.82)_42%,rgba(145,62,223,0.38)_72%,transparent_76%)] blur-[1px] ${playing ? 'animate-pulse' : ''}`} /><div className="absolute bottom-0 h-8 w-20 rounded-full bg-orange-300/15 blur-xl" /><ChristianCross /></div>
  }
  if (theme === 'Living Water') {
    return <div className="relative z-10 h-[92px] w-[100px]" aria-hidden="true"><div className={`absolute left-1/2 top-2 h-12 w-12 rounded-[55%_45%_60%_40%] border-2 border-aurora-teal/80 bg-aurora-teal/10 ${playing ? 'animate-pulse' : ''}`} style={{ transform: 'translateX(-50%) rotate(45deg)' }} /><div className="absolute bottom-4 left-2 right-2 h-1.5 rounded-full bg-aurora-teal/40" /><div className="absolute bottom-0 left-4 right-4 h-1 rounded-full bg-nebula-violet/35" /><div className="absolute bottom-8 left-1/2 -translate-x-1/2 scale-[0.65]"><ChristianCross /></div></div>
  }
  if (theme === 'Heavenly Glory') {
    return <div className="relative z-10 flex h-[92px] w-[104px] items-center justify-center" aria-hidden="true"><div className={`absolute h-20 w-20 rounded-full bg-[radial-gradient(circle,rgba(255,241,183,0.42),rgba(139,111,232,0.18)_48%,transparent_72%)] ${playing ? 'animate-pulse' : ''}`} /><div className="absolute top-3 h-3 w-12 rounded-[50%] border-2 border-comet-gold/70" /><div className="absolute bottom-3 left-1 h-6 w-12 rounded-full bg-white/10 blur-sm" /><div className="absolute bottom-2 right-0 h-7 w-14 rounded-full bg-white/10 blur-sm" /><ChristianCross /></div>
  }
  if (theme === 'Light & Truth') {
    return <div className="relative z-10 flex h-[92px] w-[104px] items-center justify-center" aria-hidden="true"><div className={`absolute h-24 w-24 bg-[conic-gradient(from_0deg,transparent,rgba(79,216,196,0.2),transparent,rgba(245,205,112,0.18),transparent)] ${playing ? 'animate-spin' : ''}`} style={{ animationDuration: '14s' }} /><div className="absolute h-20 w-2 rounded-full bg-comet-gold/10 blur-sm" /><ChristianCross /></div>
  }
  if (theme === 'Grace & Mercy') {
    return <div className="relative z-10 flex h-[92px] w-[104px] items-center justify-center" aria-hidden="true"><div className={`absolute h-20 w-20 rounded-full border border-white/15 bg-[radial-gradient(circle,rgba(255,255,255,0.14),rgba(139,111,232,0.12)_55%,transparent_72%)] ${playing ? 'animate-pulse' : ''}`} /><div className="absolute left-1 top-5 h-6 w-10 rotate-[-18deg] rounded-[100%_0_100%_0] border border-white/25" /><div className="absolute right-1 top-5 h-6 w-10 rotate-[18deg] rounded-[0_100%_0_100%] border border-white/25" /><ChristianCross /></div>
  }
  return <div className="relative z-10 flex h-[92px] w-[104px] items-center justify-center" aria-hidden="true"><div className={`absolute h-24 w-24 rounded-full border border-aurora-teal/20 ${playing ? 'animate-ping' : ''}`} /><div className={`absolute h-16 w-16 rounded-full bg-nebula-violet/20 blur-xl ${playing ? 'animate-pulse' : ''}`} /><div className="absolute bottom-1 left-0 right-0 h-8 bg-[linear-gradient(160deg,transparent_0_28%,rgba(139,111,232,0.18)_29%_52%,transparent_53%),linear-gradient(25deg,transparent_0_38%,rgba(79,216,196,0.12)_39%_58%,transparent_59%)]" /><ChristianCross /></div>
}
